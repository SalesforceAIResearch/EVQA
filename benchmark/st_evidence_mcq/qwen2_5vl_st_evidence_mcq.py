#!/usr/bin/env python3
"""
Independent Task Video Question Answering for ST-Evidence MCQ using Qwen2.5-VL
...
- all: Run all three tasks sequentially (task by task with batching)
...
Usage:
    # Run video QA task
    python qwen2_5vl_st_evidence_mcq.py --task qa --split val --model Qwen/Qwen2.5-VL-7B-Instruct
    
    # Run time evidence selection task
    python qwen2_5vl_st_evidence_mcq.py --task time_evidence --split val --model Qwen/Qwen2.5-VL-7B-Instruct
    
    # Run spatial evidence selection task
    python qwen2_5vl_st_evidence_mcq.py --task spatial_evidence --split val --model Qwen/Qwen2.5-VL-7B-Instruct
    
    # Run all tasks sequentially (saves to single unified file)
    python qwen2_5vl_st_evidence_mcq.py --task all --split val --model Qwen/Qwen2.5-VL-7B-Instruct --batch_size 4
    # This creates: result/qwen2_5/{model_name}_all_{split}_{fps}fps.json
    # with format: {"entry_id": {"answer": "A", "evidence_t": "B", "evidence_s": "C", ...}}
    
    # Custom model and FPS with batching
    python qwen2_5vl_st_evidence_mcq.py --task qa --split train --model Qwen/Qwen2.5-VL-72B-Instruct --fps 2 --batch_size 8
...
def main():
    parser = argparse.ArgumentParser(description="Independent Task Video Question Answering with Qwen2.5-VL")
    ...
    parser.add_argument('--model', type=str, default='Qwen/Qwen2.5-VL-7B-Instruct', help='Qwen2.5-VL model to use')
    ...
    parser.add_argument('--save_file', type=str, default=None, help='Save file (default: result/qwen2_5/{model_name}_{task}_{split}_{fps}fps.json)')
    ...
    parser.add_argument('--batch_size', type=int, default=1, help='Batch size for inference (default: 1 for sequential processing)')
    ...
    # Create sampling params (Official Qwen2.5-VL parameters)
    ...
    if args.save_file:
        ...
        save_file = f"{results_base}/qwen2_5/{model_name}_{task_name}_{args.fps}fps.json"
