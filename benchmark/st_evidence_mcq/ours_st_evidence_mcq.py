#!/usr/bin/env python3
"""
Independent Task Video Question Answering for ST-Evidence MCQ using UniPixel with LoRA
...
This script uses the LoRA model loading approach (is_trainable=False, merge_adapter=False).

Usage:
    python ours_st_evidence_mcq.py --task qa --model /path/to/lora/model
    python ours_st_evidence_mcq.py --task all --model /path/to/lora/model
...
def load_video_for_llm(video_path, fps, max_frames, num_threads):
    """Load video frames for LLM processing."""
    ...
    if fps is not None:
        ...
        sample_frames = int(video_duration * fps)
        sample_frames = max(1, min(sample_frames, min(max_frames, total_frames)))
    else:
...
def generate_text_response(model, processor, images, prompt, device):
...
def generate_text_response_with_images(model, processor, video_images, mask_images_pil, prompt, device):
    """Generate text response from the model with video and additional images."""
    num_frames = len(video_images)
    
    # Build content: video first, prompt, then mask images
    content = [{
        'type': 'video',
        'video': video_images,
        'min_pixels': 128 * 28 * 28,
...
def process_qa_task(model, processor, video_path: str, question: str, 
                     options: list, entry_id: str, fps: float, max_frames: int, num_threads: int, device):
    """Process Video QA task."""
    if not os.path.exists(video_path):
        print(f"✗ Video file not found: {video_path}")
        return None, f"Video file not found: {video_path}"
    try:
        frames, images = load_video_for_llm(video_path, fps, max_frames, num_threads)
        prompt = create_qa_prompt(question, options)
        response = generate_text_response(model, processor, images, prompt, device)
...
def process_time_evidence_task(model, processor, video_path: str, question: str,
                                 time_segments: list, entry_id: str, fps: float, max_frames: int, num_threads: int, device):
...
def process_spatial_evidence_task(model, processor, video_path: str, 
                                   mask_images: list, question: str, entry_id: str, 
                                   fps: float, max_frames: int, num_threads: int, device):
    ...
    try:
        # Load video
        frames, video_images = load_video_for_llm(video_path, fps, max_frames, num_threads)
        ...
        for img_path in mask_images:
            ...
            try:
                ...
                mask_images_pil.append(img)
            except Exception as e:
                print(f"⚠️  Warning: Failed to load image {img_path}: {e}")
        
        if len(mask_images_pil) == 0:
        ...
        response = generate_text_response_with_images(model, processor, video_images, mask_images_pil, prompt, device)
        answer = parse_answer(response)
        print(f"✅ Response for {entry_id}: {response}")
        return {'answer': answer}, None
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Independent Task Video Question Answering with UniPixel (LoRA)")
    parser.add_argument('--task', type=str, required=True, choices=['qa', 'time_evidence', 'spatial_evidence', 'all'],
                        help='Task type')
    parser.add_argument('--model', type=str, default='PolyU-ChenLab/UniPixel-3B', help='UniPixel model path or HuggingFace model ID')
    ...
    parser.add_argument('--num_threads', type=int, default=8, help='Number of threads for video decoding')
    ...
    # Initialize model with LoRA parameters (matching ours_st_evidence.py)
    print(f"\n🔄 Loading {args.model} with LoRA configuration...")
    ...
    # Save file (use ours directory and add model prefix)
    results_base = 'result'
    if args.save_file:
        ...
        # Add model size prefix (3b or 7b)
        if '3b' in args.model.lower():
            model_name = f'3b_{model_name}'
        elif '7b' in args.model.lower():
            model_name = f'7b_{model_name}'
        
        task_name = 'all' if unified_results else args.task
        save_file = f"{results_base}/ours/{model_name}_{task_name}_{args.fps}fps.json"
    os.makedirs(os.path.dirname(save_file), exist_ok=True)
        ...
        for task in tasks_to_run:
            try:
                ...
                if task == 'qa':
                    ...
                    result, error = process_qa_task(model, processor, video_path, question, options, entry_id, args.fps, args.max_frames, args.num_threads, device)
                    ...
                    result, error = process_time_evidence_task(model, processor, video_path, question, time_segments, entry_id, args.fps, args.max_frames, args.num_threads, device)
                    ...
                    result, error = process_spatial_evidence_task(model, processor, video_path, mask_images, question, entry_id, args.fps, args.max_frames, args.num_threads, device)
                ...
                # Save after each task (thread-safe for multi-GPU)
