#!/usr/bin/env python3
"""
Independent Task Video Question Answering for ST-Evidence MCQ using SA2VA
...
Usage:
    python sa2va_st_evidence_mcq.py --task qa --split val
    python sa2va_st_evidence_mcq.py --task all --split val
...
import sys
# NOTE: Add Sa2VA to PYTHONPATH before running this script:
#   export PYTHONPATH=/path/to/Sa2VA:$PYTHONPATH
# Or use: sys.path.append('/path/to/Sa2VA')
import torch
...
def load_video_frames(video_path, fps, max_frames, max_size):
    """Load and sample video frames."""
    reader = VideoReader(video_path)
    total_frames = len(reader)
    video_fps = reader.fps  # SA2VA VideoReader uses .fps attribute
    
    # Generate sample indices
    sample_indices = generate_sample_indices(total_frames, target_fps=fps, video_fps=video_fps, max_frames=max_frames)
    
    # Load frames
    frames = []
    for idx in sample_indices:
        frame = reader[idx]  # Returns numpy array in BGR format
        frame_rgb = frame[..., ::-1]  # Convert BGR to RGB
        pil_frame = Image.fromarray(frame_rgb)
        if max_size:
            pil_frame = resize_frame(pil_frame, max_size)
...
def generate_text_response(model, processor, frames, prompt, frame_indices=None):
...
def generate_text_response_with_images(model, processor, video_frames, mask_images_pil, prompt, frame_indices=None):
    """Generate text response from SA2VA model with video frames and additional mask images.
    
    Since SA2VA's predict_forward primarily handles video, we'll combine the mask images with video frames.
    """
    try:
        # Combine video frames and mask images
        # SA2VA expects a list of PIL images as video
        all_frames = video_frames + mask_images_pil
...
def process_qa_task(model, processor, video_path: str, question: str, 
    ...
    try:
        frames, frame_indices = load_video_frames(video_path, fps, max_frames, max_size)
        prompt = create_qa_prompt(question, options)
        response = generate_text_response(model, processor, frames, prompt, frame_indices)
...
def process_spatial_evidence_task(model, processor, video_path: str, 
                                   ...
                                   fps: float, max_frames: int, max_size: int):
    ...
    try:
        # Load video frames
        video_frames, frame_indices = load_video_frames(video_path, fps, max_frames, max_size)
        ...
        response = generate_text_response_with_images(model, processor, video_frames, mask_images_pil, prompt, frame_indices)
...
def main():
    parser = argparse.ArgumentParser(description="Independent Task Video Question Answering with SA2VA")
    parser.add_argument('--task', type=str, required=True, choices=['qa', 'time_evidence', 'spatial_evidence', 'all'],
                        help='Task type')
    parser.add_argument('--model', type=str,
                        default='models/Sa2VA-Qwen2_5-VL-7B',
    ...
    parser.add_argument('--max_size', type=int, default=512, help='Maximum size for frame/image resizing')
    ...
    if args.save_file:
        ...
        model_name = os.path.basename(args.model).replace('-', '_').lower()
        task_name = 'all' if unified_results else args.task
        save_file = f"{results_base}/sa2va/{model_name}_{task_name}_{args.fps}fps.json"
