#!/usr/bin/env python3
"""
Spatial-Temporal Evidence Analysis using VideoLLaMA3
Three-stage questioning: answer, temporal segments, referring expressions
Supports multi-turn and single-turn modes

Supported models:
- DAMO-NLP-SG/VideoLLaMA3-7B
- DAMO-NLP-SG/VideoLLaMA3-13B (if available)

Usage:
python videollama3_st_evidence.py --model DAMO-NLP-SG/VideoLLaMA3-7B --mode multi-turn --fps 1
python videollama3_st_evidence.py --model DAMO-NLP-SG/VideoLLaMA3-7B --mode single-turn --fps 1
"""
...
def process_multi_turn_item(video_path, question, options, model, processor, fps=1.0, max_frames=128):
    ...
    try:
        ...
        answer_response = processor.batch_decode(output_ids, skip_special_tokens=True)[0].strip()
        ...
        # Add assistant's answer and new user question to conversation
        ...
        output_ids = model.generate(**inputs, max_new_tokens=512)
        segments_response = processor.batch_decode(output_ids, skip_special_tokens=True)[0].strip()
        ...
        output_ids = model.generate(**inputs, max_new_tokens=512)
        ref_expr_response = processor.batch_decode(output_ids, skip_special_tokens=True)[0].strip()
...
def process_single_turn_item(video_path, question, options, model, processor, fps=1.0, max_frames=128):
    ...
    try:
        ...
        conversation = [
            {"role": "system", "content": "You are a helpful assistant."},
            {
                ...
                "content": [
                    {"type": "video", "video": {"video_path": video_path, "fps": fps, "max_frames": max_frames}},
                    {"type": "text", "text": simultaneous_prompt},
                ]
            },
        ]
...
def main():
    parser = argparse.ArgumentParser(description="Three-stage spatial-temporal evidence analysis with VideoLLaMA3")
    parser.add_argument('--model', type=str, 
                        default='DAMO-NLP-SG/VideoLLaMA3-7B',
                        help='Model name (default: DAMO-NLP-SG/VideoLLaMA3-7B)')
    ...
    save_file = f'results/videollama3/{model_short_name}_st_evidence_{args.mode.replace("-", "_")}_{args.fps}fps.json'
