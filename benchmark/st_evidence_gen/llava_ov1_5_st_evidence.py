#!/usr/bin/env python3
"""
Spatial-Temporal Evidence Analysis using LLaVA-OneVision-1.5
Three-stage questioning: answer, temporal segments, referring expressions
Supports multi-turn and single-turn modes

Supported models:
- lmms-lab/LLaVA-OneVision-1.5-8B-Instruct
- lmms-lab/LLaVA-OneVision-1.5-13B-Instruct (if available)

Usage:
python llava_ov1_5_st_evidence.py --model lmms-lab/LLaVA-OneVision-1.5-8B-Instruct --mode multi-turn --fps 1
python llava_ov1_5_st_evidence.py --model lmms-lab/LLaVA-OneVision-1.5-8B-Instruct --mode single-turn --fps 1
"""
...
def extract_video_frames(video_path, fps=1.0, max_frames=64, max_size=None):
    ...
    Args:
        ...
        max_size: If set, resize frames so the larger dimension equals max_size while maintaining aspect ratio
    ...
    for frame_idx in frame_indices:
        ...
        # Resize if requested (to reduce memory usage) while maintaining aspect ratio
...
def process_multi_turn_item(video_path, question, options, model, processor, fps=1.0, max_frames=64, max_size=None):
    ...
    try:
        ...
        answer_response = processor.batch_decode(
        ...
        # Add assistant's answer and new user question (text only, frames already in context)
        messages.append({"role": "assistant", "content": [{"type": "text", "text": answer_response}]})
        messages.append({"role": "user", "content": [{"type": "text", "text": temporal_prompt}]})
        ...
        segments_response = processor.batch_decode(
            generated_ids_trimmed, skip_special_tokens=True, clean_up_tokenization_spaces=False
        )[0].strip()
        ...
        messages.append({"role": "assistant", "content": [{"type": "text", "text": segments_response}]})
        messages.append({"role": "user", "content": [{"type": "text", "text": ref_expr_prompt}]})
        ...
        ref_expr_response = processor.batch_decode(
            generated_ids_trimmed, skip_special_tokens=True, clean_up_tokenization_spaces=False
        )[0].strip()
...
def process_single_turn_item(video_path, question, options, model, processor, fps=1.0, max_frames=64, max_size=None):
    ...
    try:
        ...
        for frame in frames:
            ...
            if max_size is not None:
                image_dict["max_pixels"] = max_size * max_size
            content.append(image_dict)
        content.append({"type": "text", "text": simultaneous_prompt})
...
def main():
    parser = argparse.ArgumentParser(description="Three-stage spatial-temporal evidence analysis with LLaVA-OneVision-1.5")
    parser.add_argument('--model', type=str, 
                        default='lmms-lab/LLaVA-OneVision-1.5-8B-Instruct',
                        help='Model name (default: lmms-lab/LLaVA-OneVision-1.5-8B-Instruct)')
    ...
    parser.add_argument('--max-size', type=int, default=None,
                        help='Resize frames so the larger dimension equals this value while maintaining aspect ratio (e.g., 384, 512), None keeps original size')
    ...
    save_file = f'results/llava_ov/{model_short_name}_st_evidence_{args.mode.replace("-", "_")}_{args.fps}fps.json'
