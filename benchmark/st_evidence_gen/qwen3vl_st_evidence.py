#!/usr/bin/env python3
"""
Spatial-Temporal Evidence Analysis using Qwen3VL with VLLM
...
Usage:
python qwen3vl_st_evidence.py --model Qwen/Qwen3-VL-8B-Instruct --mode multi-turn --fps 1 --batch-size 8 --gpu-num 1
python qwen3vl_st_evidence.py --model Qwen/Qwen3-VL-8B-Instruct --mode single-turn --fps 1 --batch-size 8 --gpu-num 1
"""
...
def process_multi_turn_item(video_path, question, options, llm, processor, sampling_params, fps, max_frames):
    ...
    try:
        ...
        text = processor.apply_chat_template(conversation, tokenize=False, add_generation_prompt=True)
        _, video_inputs, video_kwargs = process_vision_info(
            conversations=[conversation], return_video_kwargs=True, return_video_metadata=True
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Three-stage spatial-temporal evidence analysis with Qwen3VL")
    parser.add_argument('--model', type=str, 
                        choices=['Qwen/Qwen3-VL-4B-Instruct', 'Qwen/Qwen3-VL-8B-Instruct', 
    ...
    save_file = f'results/qwen3vl/{model_short_name}_st_evidence_{args.mode.replace("-", "_")}_{args.fps}fps.json'
